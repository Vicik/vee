...
<template>
  <el-table :data="tableData" >
    ...
    <el-table-column label="操作" width="150px">
      <template>
        <el-button size="mini" type="primary" @click="handleDeliver">{{edit.editable}}</el-button>
  ...
  methods: {
    handleDeliver() {
      if(this.edit.editable =="发货") {
        this.confirmMsg('是否确认发货', '发货成功', '取消发货')
      }else{
        this.confirmMsg('是否确认退款', '退款成功', '取消退款')
      }
    },
    handleDelete() {
      this.confirmMsg('是否确认删除', '删除成功', '取消删除')
    },
    confirmMsg(msg, successMsg, failMsg) {
      this.$confirm(msg, '提示', {
